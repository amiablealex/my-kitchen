...
POSITIVES = [
    ...
    # --- real-cooking additions (CP3), mined from examples.md ---
    ("some baby chestnut mushrooms", "Mushrooms"),   # word-qty + baby strip + alias
    ("two small red onions", "Red onion"),           # regression: word-qty strip, 'red' kept (was -> Onion)
    ("small sweet potatoes", "Sweet potato"),        # 'small' strip, 'sweet' kept
    ("fresh ginger", "Ginger"),                      # 'fresh' strip, NOT Ground ginger
    ("medium egg noodles", "Egg noodles"),
    ("brown onion", "Onion"),                        # alias
    ("dry red lentils", "Red lentils"),              # fuzzy specificity (red+lentil unique 100)
    ("greek yogurt", "Greek yoghurt"),               # fuzzy spelling variant -> confirms threshold
]
...
NEGATIVES = [
    ...
    # --- real-cooking additions (CP3) ---
    ("coconut cream", None),    # close to Coconut milk but distinct -> decline (precision)
    ("jalapenos", None),        # off-catalogue
]
